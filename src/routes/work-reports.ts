...
app.get('/task/:taskId', async (c) => {
  ...
  try {
    ...
    console.error('[work-reports GET /task/:taskId]', e.message)
    return c.json({ error: e.message || '일보 조회 실패' }, 500)
  }
})
...
app.post('/', async (c) => {
  ...
  try {
    ...
    console.error('[work-reports POST /]', e.message)
  ...
  try {
    ...
    for (const item of items) {
      ...
      try {
        ...
        console.warn('[work-reports POST /:reportId/other-works] item 저장 실패 (무시):', itemErr.message)
      }
    }
    return c.json({ ok: true })
  } catch (e: any) {
    console.error('[work-reports POST /:reportId/other-works]', e.message)
    return c.json({ error: e.message || '기타공종 저장 실패' }, 500)
  }
})
...
app.get('/other-work-types', async (c) => {
  ...
  try {
    ...
    console.error('[work-reports GET /other-work-types]', e.message)
    return c.json({ error: e.message || '기타공종 목록 조회 실패' }, 500)
  }
})
...
app.get('/volume-stats', async (c) => {
  ...
  try {
    ...
    console.error('[work-reports GET /volume-stats]', e.message)
    return c.json({ error: e.message || '물량통계 조회 실패' }, 500)
  }
})
