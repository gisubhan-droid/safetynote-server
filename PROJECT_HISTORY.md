...
> **GitHub 최신: `a61b71d`** — feat(FEAT-052+053): TBM완료 planned_date 자동갱신 + 완료 작업/공사 삭제(sysadmin전용)
> **NAS 배포 필요: `a61b71d`** — git pull 후 pm2 restart safetynote
...
| BUG-087 | 110 | 2026-07-06 | ✅ 수정 | **작업 삭제 시 FOREIGN KEY constraint failed (500)** — `tasks.ts DELETE /:id`의 `safeDelete` 체인에서 `tbm_records` 삭제 전 자식 테이블 누락: `tbm_signatures(REFERENCES tbm_records(id))`, `tbm_share_tokens(tbm_id)`, `signature_requests(ref_type='tbm')`, `notifications(ref_type='tbm')`가 먼저 삭제되지 않아 FK constraint 위반 → 500. **해결**: `tbm_records` 삭제 직전에 4개 테이블 safeDelete 순서 삽입 | `TBD` |
