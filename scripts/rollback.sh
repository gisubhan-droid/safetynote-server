...
declare -A COMMIT_MAP=(
  # ── 세션 51 (최신) ─────────────────────────────────────────
  ["s51-fix4"]="d6bc5a4"  # ★현재 — GET volume-unit-prices SELECT unit 누락 수정 (캐시 u)
  ["s51-fix3"]="2d00b56"  # 세션51 — 외선 단위 저장 버그 stmtUnit 추가 (캐시 t)
  ["s51-fix2"]="2e174b1"  # 세션51 — 외선 단가관리 컬럼 순서 수정 (공종|단가|단위, 캐시 s)
...
# ╚══════════════════════════════════════════════════════════════╝
declare -A DESC_MAP=(
  ["s51-fix4"]="★현재 — GET volume-unit-prices SELECT unit 콼럼 누락 수정 (외선 단위 표시/저장 완전 수정, 캐시 u)"
  ["s51-fix3"]="세션51 — 외선 단위 저장 버그 stmtUnit 추가 (캐시 t)"
  ["s51-fix2"]="세션51 — 외선 단가관리 컬럼 순서 공종|단가|단위 + 단위 개별수정 (캐시 s)"
...
print_versions() {
  ...
  local ORDER=(prev latest s51-fix4 s51-fix3 s51-fix2 s51-fix s51 pre-s51 s50-hotfix s50-final s50 pre-s50 s49 pre-s49 s48 pre-s48 s47 s46 pre-bug023 pre-bug022 pre-bug011-safe)
  for key in "${ORDER[@]}"; do
