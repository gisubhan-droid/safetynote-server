...
> 최종 업데이트: 2026-07-24 (세션 68-B — 잠재 문제 4개 수정: 체크리스트 사진 UX 강화 + bucket SQL 정밀화)
> **GitHub 최신: `2adfc3d`** — fix: [세션 68] 잠재 문제 4개 수정 — 체크리스트 사진 UX 강화 + bucket SQL 정밀화
...
|------|------|------|------|----------|------|
| FIX-CHECKLIST-UX + FIX-BUCKET-SQL | 68-B | 2026-07-24 | ✅ 수정 | **잠재 문제 4개 수정** — **[문제-1]** `forceCompleteChecklist()` 사진 미등록 안내 강화: 체크리스트 완료 직후 미등록 사진 수 자동 감지(API 재조회). 미등록 있으면 기존 단순 showTbmPhotoModal 대신 강화 오렌지 경고 모달(사진 수 표시 + '지금 등록하기' 버튼) 표시. **[문제-2]** 전체 na(비대상) 선택 경고: `forceCompleteChecklist()` 진입 시 DOM `.checklist-row` 순회하여 all-na 감지, 오렌지 경고 모달('다시 확인하기'/'이대로 완료') 표시. `_forceCompleteChecklistConfirmed()` 전역 함수 분리(RULE-003). **[문제-3]** `updateTbmSections()` bucket SQL 정밀화: 기존 `question LIKE '%버켓차량%' OR category IN ('충돌','전도','감전')` → `work_class = 'bucket'`으로 교체. DB 확인 결과 `감전` 카테고리(id:47,71,72)가 `work_class='pole'` 항목으로 버켓과 무관 — 오분류 수정. **[문제-4]** 사진 모달 `×` 닫기 버튼 confirm 경고: `_tbmPhotoCloseBtn()` 전역 함수 분리(RULE-003). DOM 기반 미등록 항목 수 감지 후 confirm 다이얼로그 출력. 취소 시 모달 유지. 검증: `node --check` ✅ + `npm run build` ✅(283.70 kB) | `2adfc3d` |
