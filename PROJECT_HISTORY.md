...
> 최종 업데이트: 2026-07-03 (세션 85)
> **서버 현재 버전: `TBD`** ← 커밋 후 확정 예정 — BUG-060 DB 초기화 동작 안함 수정
...
|------|------|------|------|----------|------|
| BUG-060 | 85 | 2026-07-03 | ✅ 수정 | 데이터 초기화 동작 안함 — `admin.ts` 테이블명 불일치(`tbm_sessions`→`tbm_records`) + 누락된 연쇄삭제 테이블들 추가 | `TBD` |
