...
> **이전 커밋 (safetynote-server): `2cf0cfd`** — fix: [BUG-202] 자동업데이트 후 tsx 바이너리 누락 근본 해결 — fixTsxBinary() 추가 + NAS 공통 버그 목록/체크리스트 보강
...
> **이전 커밋 (safetynote-server): `caacebc`** — feat: [세션 110] 파일 저장 체계화 — 안전교육·SC회의 폴더구조 변경 + 마이그레이션 API
...
> **이전 커밋: `b7ec1d4`** — docs: [세션 70] PROJECT_HISTORY.md 헤더 갱신 + FEAT-WT-SAFETY 기록 추가
> **이전 커밋: `3f1e4bf`** — feat: [세션 70] FEAT-WT-SAFETY — 작업유형별 안전내용 관리 메뉴 + DB 연동
> **이전 커밋: `79c66b1`** — docs: [세션 69] PROJECT_HISTORY.md 헤더 갱신 + FEAT-TBM-PREVIEW/EDU-PREVIEW/TBM-SIGN-NAME 기록 추가
> **이전 커밋: `5e54dcf`** — docs: [세션 68-B] PROJECT_HISTORY.md 헤더 갱신 + 잠재 문제 4개 수정 기록 추가
> **이전 커밋: `2adfc3d`** — fix: [세션 68] 잠재 문제 4개 수정 — 체크리스트 사진 UX 강화 + bucket SQL 정밀화
> **이전 커밋: `7da262c`** — docs: [세션 68] PROJECT_HISTORY.md 헤더 갱신 + FEAT-TBM-MEETING 기록 추가
> **이전 커밋: `f140d32`** — feat: [FEAT-TBM-MEETING] updateTbmSections() — TBM 카테고리 OK 시 'TBM 회의' 사진 섹션 자동 생성
> **이전 커밋: `b494186`** — fix: [세션 67] BUG-SCROLL — PDF/이미지 뷰어 확대 후 스크롤/드래그 이동 불가 수정
> **이전 커밋: `a7a31a6`** — docs: [세션 66] PROJECT_HISTORY.md 헤더 갱신 + BUG-PDF-ZOOM/FEAT-PDF-PINCH/FEAT-IMG-PINCH 기록 추가
> **이전 커밋: `20ea2eb`** — fix/feat: [세션 66] PDF +/- 줌 버그 수정 + 핀치 줌 구현 (PDF + 이미지)
> **이전 커밋: `6ee2ef1`** — docs: [세션 65] PROJECT_HISTORY.md 헤더 갱신 + FEAT-170b 기록 추가
> **이전 커밋: `e3f518e`** — feat: [FEAT-170b] Android 앱 인앱 PDF/이미지 뷰어 + PC 새 탭 방식 복원
...
> **이전 커밋: `7feff5e`** — fix: [BUG] GET /api/attachments NAS — attach_type 필터 추가 (work_log/order 분리)
...
> **이전 커밋: `6be1bec`** — feat: [FEAT-160] 근로자 my-stats 총 시공물량 → 팀 일보 총금액으로 변경
...
> **이전 커밋: `2605aa4`** — feat: [FEAT-158] my-stats 안전 순위 카드 추가 — 이번 달 기준 본인 순위 표시
...
| FEAT-TBM-PREVIEW + FEAT-EDU-PREVIEW + FIX-TBM-SIGN-NAME | 69 | 2026-07-24 | ✅ 구현 | **TBM/교육 결재 미리보기 버튼 추가 + _tbmSign() 서명자 이름 표시** — **[수정 A] TBM 미리보기**: `showTbmDetail()` modal-footer의 기존 '🖨 인쇄' 버튼 앞에 '📄 미리보기' 버튼 추가(초록 #059669). `_tbmPrint(tbmId)` 그대로 재활용 — 실제 인쇄 없이 A4 오버레이 미리보기만 표시. **[수정 B] 교육 결재 미리보기**: `showEduDetailModal()` 출력 버튼 바 맨 앞에 '📄 결재 미리보기' 버튼 추가(초록 #059669). `printEduSign(sessionId)` 재활용 — 서명지 A4 미리보기 오버레이로 동작. **[수정 C] _tbmSign() 서명자 이름**: 내 서명 버튼 클릭 시 서명 팝업 subtitle에 `currentUser.name` 표시(`"홍길동 님 — 서명 후 [서명 완료] 버튼을 눌러주세요"`). `currentUser` null guard 적용(이름 없을 시 기존 문구 폴백). RULE-001 준수: `const → var` 전환, 백틱 단순화 → 문자열 연결. **기존 서명 함수 이름 표시 현황**: `_tbmSignFor(personName)` ✅ 이미 표시, `_tbmCollectSign(name)` ✅ 이미 표시, `_tbmApprovalSignInApp()` ✅ 역할 설명 표시, `_eduApprovalSignInApp()` ✅ 역할 설명 표시. 변경 파일: `public/static/app.js` 단독. 검증: `node --check` ✅ + `npm run build` ✅(283.70 kB) | `0c2aefc` |
| FIX-CHECKLIST-UX + FIX-BUCKET-SQL | 68-B | 2026-07-24 | ✅ 수정 | **잠재 문제 4개 수정** `forceCompleteChecklist()` 사진 미등록 안내 강화: 체크리스트 완료 직후 미등록 사진 수 자동 감지(API 재조회). 미등록 있으면 기존 단순 showTbmPhotoModal 대신 강화 오렌지 경고 모달(사진 수 표시 + '지금 등록하기' 버튼) 표시. **[문제-2]** 전체 na(비대상) 선택 경고: `forceCompleteChecklist()` 진입 시 DOM `.checklist-row` 순회하여 all-na 감지, 오렌지 경고 모달('다시 확인하기'/'이대로 완료') 표시. `_forceCompleteChecklistConfirmed()` 전역 함수 분리(RULE-003). **[문제-3]** `updateTbmSections()` bucket SQL 정밀화: 기존 `question LIKE '%버켓차량%' OR category IN ('충돌','전도','감전')` → `work_class = 'bucket'`으로 교체. DB 확인 결과 `감전` 카테고리(id:47,71,72)가 `work_class='pole'` 항목으로 버켓과 무관 — 오분류 수정. **[문제-4]** 사진 모달 `×` 닫기 버튼 confirm 경고: `_tbmPhotoCloseBtn()` 전역 함수 분리(RULE-003). DOM 기반 미등록 항목 수 감지 후 confirm 다이얼로그 출력. 취소 시 모달 유지. 검증: `node --check` ✅ + `npm run build` ✅(283.70 kB) | `2adfc3d` |
| FEAT-TBM-MEETING | 68 | 2026-07-24 | ✅ 구현 | **TBM 카테고리 OK 응답 시 'TBM 회의' 필수 사진 섹션 자동 생성** — `updateTbmSections()`에 TBM 카테고리 항목 ID 쿼리 추가(`category='TBM' AND is_active=1`). `sectionDefs` 배열에 `tbm_meeting` 섹션 추가: `type:'tbm_meeting'` / `name:'TBM 회의'` / `labels:['TBM회의 사진','작업현장 전경','라바콘 입간판 설치상태']` / `needed:hasOk(tbmIds)`. 트리거 조건: TBM 카테고리 4개 항목(작업자간 위험요인 확인, 신규입사자 안전교육, 작업중지권 등) 중 하나라도 OK 응답 시 섹션 자동 생성. OK 응답 없을 시 섹션 자동 삭제(기존 ppe/bucket/heavy 동일 패턴). 사진 보존 로직(label 기준 upsert) 그대로 적용. 기존 3개 섹션 로직 충돌 없음 확인. 검증: `npm run build` ✅(283.81 kB) | `f140d32` |
| BUG-SCROLL-PDF + BUG-SCROLL-IMG | 67 | 2026-07-24 | ✅ 수정 | **PDF/이미지 뷰어 확대 후 스크롤/드래그 이동 불가 수정** — **[BUG-SCROLL-PDF]** `touch-action:none` → `touch-action:pan-x pan-y` 변경: 1손가락은 브라우저 기본 스크롤 허용, 2손가락 핀치 시에만 JS에서 `e.preventDefault()` 호출. `_snPinchActive` 플래그 도입: `touchstart` 2손가락→`true`/1손가락→`false`, `touchmove`는 `_snPinchActive && 2손가락`일 때만 스크롤 차단. `touchend` `passive:true` 전환. `flex-shrink:0` 추가(캔버스 수축 방지). **[BUG-SCROLL-IMG]** `overflow:hidden` → `overflow:auto` 변경. CSS `transform:scale()` 방식 폐기 → `img.width/height` 직접 조정 방식 전환(`transform:scale()`은 layout에 영향 없어 overflow 스크롤 영역 안 늘어남). `_imgApplyScale(scale)` 함수 도입: `naturalWidth/Height` 기반 fit 크기 계산 후 scale 배율로 `img.style.width/height` 직접 설정. `imgEl.onload` 콜백에서 초기 크기 설정. `_imgPinchActive` 플래그 도입(PDF와 동일 구조). 더블탭 시 `scrollLeft/scrollTop` 0 리셋 추가. 검증: `node --check` ✅ + `npm run build` ✅(283.54 kB) | `b494186` |
| BUG-PDF-ZOOM + FEAT-PDF-PINCH + FEAT-IMG-PINCH | 66 | 2026-07-24 | ✅ 구현 | **PDF +/- 버튼 줌 버그 수정 + PDF·이미지 뷰어 핀치 줌 구현** — **[BUG-PDF-ZOOM]** `_snPdfRenderPage()`의 `_snPdfRendering` 플래그 stuck 문제 수정: ①`render().promise.catch()` 추가 → 렌더 실패 시 `_snPdfRendering=false` 리셋 ②`getPage().catch()` 추가 → 페이지 로드 실패 시에도 리셋 ③`RenderingCancelledException` 무시(정상 취소) ④`canvas._snRenderTask`로 이전 renderTask 추적→취소(중복 렌더 방지). `_snPdfClose()`에 `_snPdfRendering=false` 추가 → 뷰어 닫힐 때 stuck 해제 보장. canvas `max-width:100%` 제거 → 줌 시 캔버스가 실제 해상도로 확대되고 `overflow:auto` 스크롤로 탐색 가능. **[FEAT-PDF-PINCH]** PDF 뷰어 `sn-pdf-body`에 `touchstart/touchmove/touchend` 이벤트 리스너 추가: 두 손가락 거리 비율로 scale 계산(0.5~3.0), touchmove 중 canvas CSS transform 미리보기, touchend 후 200ms 디바운스 후 `_snPdfRenderPage()` 재렌더. **[FEAT-IMG-PINCH]** 이미지 뷰어에 `touchstart/touchmove/touchend` 이벤트 리스너 추가: CSS `transform:scale()` 실시간 적용(0.5~5.0), touchend 1배 미만 자동 복원, 더블탭으로 1배 복원. **[RULE-003]** `_snImgClose()` 전역함수 분리(onclick 내 따옴표 중첩 제거). RULE-001 준수: 전부 `var`+`function`. 검증: `node --check` ✅ + `npm run build` ✅(283.54 kB) | `20ea2eb` |
...
| 사용자 설명서 (권한별) | ✅ 초안완성 | `b75293e8` — `docs/USER_GUIDE.md` (26,338 bytes, 682줄) 작성 완료. 관리자·감독자·근로자 3권한, 7개 섹션 + 부록 |
...
## 📄 세션 57 — 배포설명서 수정 (DOCS-002) (2026-07-23)

### 완료된 작업

- ✅ **DOCS-002**: `docs/DEPLOY_AND_UPDATE_GUIDE.md` 전면 개정
  - 6장 신설: **PM2 자동복구 (Watchdog) 설정** — DSM 작업 스케줄러 단계별 등록 절차
  - Watchdog 동작 원리 다이어그램 추가 (정상 → 재시작 → 롤백 → 비상복구 순서)
  - 관련 파일 경로 테이블 추가 (스크립트 / 로그 / crash 카운터 / 비상복구 PID)
  - 업데이트 방법: 방법 A(브라우저) ✅ **권장** 명시, 방법 B·C SSH → 선택사항으로 격하
  - SSH 안내: 별도 "선택사항 — SSH 접속" 섹션으로 분리 + SSH 포트 변경 보안 안내 추가
  - 타 NAS 배포: 4단계 `pm2 startup` 경고 + 6단계 Watchdog 등록 필수 안내 추가
  - 현재 상태 요약 갱신 (최신 커밋 `4fbdf3f` → PM2 자동복구 항목 ✅ 추가)
  - 장애 대응 FAQ: Watchdog 관련 Q&A 2건 추가
- ✅ **빌드 검증**: `node --check` + `npm run build` → `dist/_worker.js 283.47 kB` ✅
- ✅ **Git 커밋**: `1e38f65` — docs: [DOCS-002] 배포설명서 수정 — Watchdog 등록 절차 + SSH 선택사항 + 브라우저 업데이트 위주 개편
- ✅ **GitHub Push**: `4fbdf3f..1e38f65` → main ✅

### 수정 요약

| 항목 | 이전 | 이후 |
|------|------|------|
| SSH 안내 | 전 섹션에 혼재 | "선택사항" 독립 섹션으로 분리 |
| 업데이트 방법 | A·B·C 동등 나열 | A(브라우저) ✅ 권장 강조 |
...
## 세션155j — 사진대장 sida-lbl 완전 제거 + 깔끔한 2열 레이아웃 (2026-07-22)
...
#### 1. 좌측 "사진대지" 라벨 완전 제거
- `.sida-lbl` CSS 클래스 삭제
- `colgroup` 3열(28px + 사진2열) → **2열(50% : 50%)** 단순화
- `rowspan` sida-lbl td 전부 제거

#### 2. CSS 전면 개선
| 클래스 | 변경 내용 |
|--------|----------|
| `.photo-page` | `padding:6mm 8mm 4mm 8mm`, `box-sizing:border-box` 추가 |
| `.photo-cell` | `padding:0`, `background:#eee` (빈 셀 회색 배경) |
| `.caption-cell` | `text-align:left`, `padding:4px 8px`, `line-height:1.4`, `font-weight:500` |
| `.photo-sec-hdr` (신규) | 네이비(`#1E3A5F`) 배경 + 흰색 글씨 헤더 |
| `.photo-divider` (신규) | `height:5mm`, 연파랑 배경 구분선 |
...
#### 4. A4 비율 계산
- A4 유효 사진구역: 297 - 패딩10 - 제목10 - 헤더38 = 239mm
- 상단: 헤더8 + 사진95 + 캡션12 = 115mm
- 구분: 5mm
- 하단: 헤더8 + 사진95 + 캡션12 = 115mm
- **합계: 235mm** ✅
...
## 세션155l — 체크리스트 섹션 헤더 전체 해당없음 체크박스 (2026-07-22)
...
#### 1. 등록 모달 섹션 헤더 변경 (`_insRegChkHtml` 빌더, line ~16044)
- 기존: 단순 `<div style="background:#685182...">sec.group</div>`
- 변경: flex + justify-content:space-between → 좌측 섹션명, 우측 "전체 해당없음" 체크박스
- 체크박스: `accent-color:#fff`, `data-secgrp=encodeURIComponent(sec.group)`, `onchange="_setInsRegSecAllNa(this)"`

#### 2. 수정 탭 섹션 헤더 변경 (`_renderInsChkTab`, line ~16964)
- 기존: 단순 `<div style="background:#e8e0f0;border-left:4px solid #685182...">sec.group</div>`
- 변경: flex + justify-content:space-between → 동일 패턴
- 체크박스: `accent-color:#685182`, `data-secgrp=encodeURIComponent(sec.group)`, `data-ins=insId`, `onchange="_setInsSecAllNa(this)"`

#### 3. 신규 함수 `_setInsRegSecAllNa(cb)` (등록 모달용)
- `data-secgrp` getAttribute → decodeURIComponent
- `cb.checked` false 시 즉시 return (체크 해제 시 자동 복원 없음)
- `_INS_CHECKLIST` 순회 → 섹션 일치 시 항목별 na 버튼 querySelector
- `_setInsRegChk(key, 'na', naBtn)` 호출 → 기존 사진 슬롯 숨김 로직도 자동 동작

#### 4. 신규 함수 `_setInsSecAllNa(cb)` (수정 탭용)
- `data-secgrp` + `data-ins` 읽기
- 동일 패턴으로 `_setInsChk(insId, key, 'na', naBtn)` 호출

#### 충돌 없음 확인
| 항목 | 결과 |
|------|------|
| `_setInsRegChk` / `_setInsChk` 기존 함수 그대로 재사용 | ✅ |
| `var` 전용 (const/let/?.  없음) | ✅ |
| 백틱 중첩 없음 | ✅ |
...
## 세션 80 (2026-07-25) — DOCS-001 사용자 설명서 (권한별) 작성 + 버전 태깅 수정

### 작업 내용

#### 버전 태깅 수정
- PROJECT_HISTORY.md 전체 `서버 v1.0.0` → `서버 V2.0.0` 일괄 수정 (3개소)
- 최종 릴리즈 목표: **서버 V2.0.0 + APK v2.0.0**

#### DOCS-001 사용자 설명서 (권한별) 작성

**파일**: `docs/USER_GUIDE.md` — 신규 작성 (26,338 bytes, 682줄, 7개 섹션 + 부록 2개)

**권한 구조 분석 기반 (app.js 45,189줄 실제 코드 분석):**

| DB 권한 | UI 역할 | 메뉴 그룹 | 비고 |
|---------|---------|----------|------|
| `admin` | CEO / 시스템관리자 | allManagerGroups (5그룹) | 계정관리 + 단가관리 포함 |
| `supervisor` | 안전관리자 / 공무 / 현장대리인 | allManagerGroups (5그룹) | 계정관리·단가관리 제외 |
| `worker` | 근로자 | workerGroups (3그룹) | 내 작업·위험신고·서명요청·내 계정만 |
| `lgu_plus` | LGU+ | lguGroups (가변) | 관리자 설정에 따라 메뉴 on/off |

**문서 구조:**

| 섹션 | 내용 |
|------|------|
| 1. 권한 구조 한눈에 보기 | 3단계 권한 비교표, 아이콘 색상 안내 |
| 2. 공통 — 앱 설치 및 첫 로그인 | APK 설치, 브라우저 접속, 로그인 화면 |
| 3. 공통 — 화면 구성 및 내비게이션 | 레일/플라이아웃 구조, 권한별 메뉴 그룹 비교 |
| 4. 관리자 (CEO·시스템관리자) 전용 | 계정관리, 팀관리, 단가관리, 시스템설정 8탭, 교육관리 |
| 5. 감독자 가이드 | 현장점검, 위험신고 처리, 서명요청 발송, 작업관리, 통계 |
| 6. 근로자 가이드 | 내 작업, 상태변경, 작업일보, 위험신고, 서명처리, 내 계정 |
| 7. 공통 FAQ | 로그인 오류 8가지, GPS·사진·알림·비밀번호 문제 |
...
| repo | commit | 내용 |
|------|--------|------|
| safetynote-server | `b75293e8` | docs: [DOCS-001] 사용자 설명서 권한별 가이드 작성 (USER_GUIDE.md) |
| safetynote-server | `04d3b99` | docs: [세션79-2] DOCS-001 NAS_INSTALL_GUIDE.md 초안 작성 완료 |

### Phase 4 현재 상태

| 항목 | 상태 | 내용 |
|------|------|------|
| NAS 설치 가이드 | ✅ **보강완성** | `7e0bad1c` — `docs/NAS_INSTALL_GUIDE.md` v2.0 (971줄, 33,972 bytes) — update/reinstall 모드, patchSchema, Watchdog 상세, 섹션 10 신규 |
| 사용자 설명서 (권한별) | ✅ **보강완성** | `1ebee708` — `docs/USER_GUIDE.md` v2.0 (1,056줄, 40,659 bytes) — 시스템설정 8탭 전체 상세, 감독자 비교표, LGU+ 섹션 7, 부록C |
| 배포설명서 | ✅ 완료 | `1e38f65` — DOCS-002 |
| PDF + 스크린샷 형식 | 🔲 미착수 | 차기 작업 예정 |
...
## 세션 81 (2026-07-25) — Phase 4 문서 내용 보강 완료 (옵션 A)

### 작업 내용

#### 1. NAS_INSTALL_GUIDE.md v2.0 보강 (736줄 → 971줄, 25,063 → 33,972 bytes)

**추가 섹션 및 항목:**

| 위치 | 추가 내용 |
|------|-----------|
| 섹션 3 Step A-4 이후 | patchSchema v0.171 안내 박스 신규 추가 (DROP 없이 ADD만, 롤백 안전) |
| Step B-6 | `--cwd "$INSTALL_DIR"` 필수 옵션 경고 명시 (FIX-052) |
| 섹션 7 Watchdog | crash 카운터 포함 상세 동작 다이어그램, 관련 파일 경로 테이블 추가 |
| **섹션 10 (신규)** | 업데이트 및 재설치 가이드 전체 |
| 섹션 10-1 | fresh/update/reinstall 3가지 모드 비교표 |
| 섹션 10-2 | 브라우저 원클릭 업데이트 절차 |
| 섹션 10-3 | install.sh 명령행 업데이트 절차 |
| 섹션 10-4 | patchSchema 자동 적용 상세 (동작 원리, 확인 로그, 스키마 오류 대응) |
| 섹션 11 FAQ | 신규 케이스 2개 — "업데이트 후 서버 미시작 (DB 오류)", "Watchdog 반복 재시작" |
| 섹션 12 운영요약 | crash 카운터 확인 명령 추가 |

#### 2. USER_GUIDE.md v2.0 보강 (682줄 → 1,056줄, 26,338 → 40,659 bytes)

**추가 섹션 및 항목:**

| 위치 | 추가 내용 |
|------|-----------|
| 섹션 1 | 권한 구조 텍스트 다이어그램 (DB role 계층) |
| 섹션 3~4 | FCM 알림 설명 추가 |
| 섹션 4-4 | 단가 관리 등록 방법 상세 (① 단가 관리 탭 → [+ 단가 추가] 흐름) |
| **섹션 4-5 시스템설정 8탭 (전체 신규)** | 탭 1~8 각각 상세 사용법 |
| └ 탭 1 푸시 알림 발송 | 전체/그룹/개인 발송 방법, 알림 내역 확인 |
| └ 탭 2 파일 설정 | 업로드 경로 설정, 파일 목록 관리 |
| └ 탭 3 GPS 주소 변환 | 카카오 API 키 초기 설정 1회 절차 (5단계) |
| └ 탭 4 APK 배포 관리 | URL 등록 vs 직접 업로드 두 방법 비교 |
| └ 탭 5 서버 업데이트 | 실시간 로그 출력 흐름, 롤백 패널 사용법 |
| └ 탭 6 그룹별 권한 설정 | ON/OFF 토글 UI 레이아웃, 적용 범위 |
| └ 탭 7 LGU+ 권한 설정 | 기본값 비허용 상태 명시, 허용 절차 |
| └ 탭 8 정보 | 표시 항목 전체 목록 |
| **섹션 5-2 (신규)** | 감독자 역할별 실무 차이 비교표 (안전관리자/공무/현장대리인) |
| 섹션 5-5 서명요청 | 현황 테이블 UI 다이어그램, 재발송 기능 설명 추가 |
| 섹션 6-2 내 작업 | 작업 카드 UI 다이어그램 추가 |
| 섹션 6-3 | 작업 시작 흐름 상세 추가 |
| **섹션 7 (신규)** | LGU+ 사용자 가이드 (3개 소섹션) |
| 섹션 8 FAQ | Q9 서명 완료 확인 방법, Q10 알림 vs 앱 내용 불일치 신규 추가 |
| 부록 A | LGU+ 열 추가 (4열 비교표로 확장) |
| 부록 B | 사용자별 접속 방법 테이블 추가 |
| **부록 C (신규)** | 자주 사용하는 메뉴 경로 요약 (관리자/감독자/근로자 3개 테이블) |

#### 3. GitHub API 업로드

| 파일 | 이전 SHA | 새 SHA | 크기 |
|------|----------|--------|------|
| `docs/NAS_INSTALL_GUIDE.md` | `2b762e06` | `7e0bad1c` | 33,972 bytes |
| `docs/USER_GUIDE.md` | `9253d354` | `1ebee708` | 40,659 bytes |

### 커밋

| repo | commit | 내용 |
|------|--------|------|
| safetynote-server | (이번 세션 커밋 예정) | docs: [세션81] Phase 4 문서 내용 보강 완료 |

### Phase 4 최종 상태

| 항목 | 상태 | 내용 |
|------|------|------|
| NAS 설치 가이드 | ✅ **보강완성** | `7e0bad1c` — `docs/NAS_INSTALL_GUIDE.md` v2.0 (971줄, 33,972 bytes) |
| 사용자 설명서 (권한별) | ✅ **보강완성** | `1ebee708` — `docs/USER_GUIDE.md` v2.0 (1,056줄, 40,659 bytes) |
| 배포설명서 | ✅ 완료 | `1e38f65` — DOCS-002 |
| PDF + 스크린샷 형식 | 🔲 미착수 | 차기 작업 예정 |
...
| 우선순위 | ID | 항목 | 내용 | 비고 |
|---------|-----|------|------|------|
| 낮음 | Phase 3 | **node-server.ts 라우트 파일 분리** | 코드 구조 정리 — rawDb 공유 모듈 분리, 라우트 파일 9개 생성 | 기능 변화 없음, 운영 불편 없으면 후순위 |
...
### 작업: FEAT-216-2 — 모바일 지도 비율 조정

**배경**  
실기기(갤럭시 안드로이드) 확인 결과 하단 리스트가 카드 1개만 표시됨.  
지도 높이를 줄이고 리스트 높이를 늘려 카드 2~3개 표시 가능하도록 조정.  
드롭다운 폰트도 1px 축소하여 필터 바 전체 높이 최소화.

**변경 내용** (`public/static/style.css` — 모바일 미디어쿼리 내부만)

| 항목 | 변경 전 | 변경 후 |
|------|---------|---------|
| `#siteMapList` (≤768px) | `130px` | `185px` (+55px) |
| `#leafletMap min-height` (≤768px) | `150px` | `120px` (-30px) |
| `#siteMapList` (≤480px) | `110px` | `155px` (+45px) |
| `#leafletMap min-height` (≤480px) | `120px` | `100px` (-20px) |
| 드롭다운/버튼 font-size | `12px` | `11px` (CSS 오버라이드) |

**PC/태블릿/대형 화면 값 변경 없음**  
**검증**: `npm run build` ✅ (`dist/_worker.js 298.71 kB`, 1.29s)

**캐시버스팅**: `style.css?v=20260807g`  
**커밋**: `1b2e412` — fix: [FEAT-216] 모바일 지도 비율 조정 — siteMapList 높이 확대 + 드롭다운 폰트 축소 (v=20260807g)
