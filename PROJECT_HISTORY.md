...
### 다음 단계
- **NAS git pull + pm2 restart** — GitHub push 반영 (아래 명령어 참고)
- **동작 확인** — 작업 상세/공사 상세 화면에서 서브작업번호 표시 확인
