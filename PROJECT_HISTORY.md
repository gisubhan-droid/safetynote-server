...
> 최종 업데이트: 2026-07-04 (세션 87)
> **서버 현재 버전: `TBD`** ← 최신 (GitHub) — BUG-064 접속단가 CSV 업로드 400 수정
...
|------|------|------|------|----------|------|
| BUG-064 | 87 | 2026-07-04 | ✅ 수정 | 접속단가 CSV 업로드 400 에러 — `iPrice` 파싱 조건 `기본` 포함 추가(`기본(원)` 인식), `iAerial` 조건 `신호수` 포함 추가(`신호수배치(원)` 인식) / export 헤더 `가공추가금액(원)` → `신호수배치(원)` / UI `가공 추가금액` → `신호수배치 추가금액` | `TBD` |
