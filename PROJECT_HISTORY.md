...
> 최종 업데이트: 2026-07-04 (세션 91)
> **서버 현재 버전: `TBD`** ← 최신 (GitHub) — BUG-067/068 단가관리 수동추가 열순서 수정 + 헤더 sticky + 공종명 미입력 강조
...
|------|------|------|------|----------|------|
| BUG-068 | 91 | 2026-07-04 | ✅ 수정 | 단가관리 테이블 헤더 스크롤 시 사라짐 — `overflow-y:auto` + `max-height:60vh` + `thead position:sticky top:0` 적용 (외선·접속 모두) | `TBD` |
| BUG-067 | 91 | 2026-07-04 | ✅ 수정 | 수동 추가 시 공종명이 공종키 열에 입력됨 — `_upAddCableItem`·`_upAddSpliceItem` DOM 삽입 코드를 FEAT-039 열 구조(공종키\|공종명\|단위\|단가\|삭제)에 맞게 수정 / 공종명=공종키인 행 주황색 강조+안내 | `TBD` |
